...
def lister_eleves_classe(id_classe: int, db: Session = Depends(get_db), admin: models.AdminEcole = Depends(admin_ecole_courant)):
    ...
    visuelle. Volontairement séparé du listing général (plus léger, sans photo), car
    limité à une classe à la fois."""
    _verifier_classe_dans_ecole(db, id_classe, admin.id_ecole)
    eleves = db.query(models.Eleve).filter(models.Eleve.id_classe == id_classe).order_by(models.Eleve.nom).all()
    return [
