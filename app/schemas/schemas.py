...
class EleveCreation(BaseModel):
    ...
    photo_base64: Optional[str] = Field(default=None, max_length=800000)


class PhotoEleve(BaseModel):
    photo_base64: str = Field(max_length=800000)
...
class NoteGrilleEntree(BaseModel):
    id_eleve: int
    valeur: float


class NoteGrilleEnregistrement(BaseModel):
    id_classe: int
    id_matiere: int
    type_evaluation: str
    numero: int
    notes: list[NoteGrilleEntree]
